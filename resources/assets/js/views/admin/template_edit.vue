                        ...
                        <div class="m-portlet__body">
                            <div class="m-form__section m-form__section--first">
                                ...
                                <div class="form-group m-form__group row" :class="errors.has('message') || validationErrors.message ? 'has-error' : ''">
                                    ...
                                    <div class="col-lg-6">
                                        <textarea data-vv-as="Message" name="message" rows="4" cols="50" v-validate="'required|max:'+data.sms_text_size"  v-model="template.message" type="text" class="form-control m-input" placeholder="Enter Message" /></textarea>
...
export default {
  ...
  mounted(){
      if(this.$route.params.id){ 
          ...
          this.bindCurrentRoute("AdminTemplateList");
      }
  },
  computed: {
    charactersLeft() {
      ...
      partsLeft() {
          var parts = this.template.message.length,limit = this.data.sms_text_part, part_size = this.data.sms_text_part_size;
  ...
  methods: {
    ...
    updateTemplate() {
        ...
        this.$validator.validateAll().then((result) => { 
            if(result == true){
                ...
                axios.post(BASE_URL+"admin-api/templates/"+this.$route.params.id, this.template).then((res) => 
                {                    
    ...
    getTemplateById(id) {
        let vm = this;
        var refUrl = BASE_URL+'admin-api/templates/'+id+'/edit';
        if(typeof commonLib != 'undefined'){
            commonLib.blockUI({target: ".m-content",animate: true,overlayColor: 'none'});
        }

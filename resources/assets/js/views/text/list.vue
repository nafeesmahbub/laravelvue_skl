                                    ...
                                    <div class="m-form m-form--fit">
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12">
                                                ...
                                                &nbsp;&nbsp;
                                                <button class="btn btn-secondary m-btn m-btn--icon" @click="fetchTextList('api/inbound-list?page=1&start_time='+searchKey.start_time+'&end_time='+searchKey.end_time),resetsearchKey()" id="m_reset">
                                                    <span>
                                ...
                                <table class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline collapsed sortable" id="m_table_1" role="grid" aria-describedby="m_table_1_info" style="width: 1044px;">
                                    ...
                                    <tbody>
                                        <tr v-for="(log, index) in data.data" :class="log.status == 'U' ? 'm-bold' : ''" style="text-align: center">                                            
                                            ...
                                            <td>
                                                <a href="#" class="btn btn-info btn-reply" @click.prevent="bindModalData(log, index)" data-toggle="modal" data-target="#text-modal" title="Read">
                                                    Read
                                                </a>
                                                <a href="#" class="btn btn-primary btn-reply" data-toggle="m-tooltip" @click.prevent="routeToInbox(log, index)" title="Reply">
                                                    Reply
                                                </a>
                                                <!-- <router-link @click.prevent="bindModalData(log, index)" href="#"  v-bind:to="{name: 'InboxList', params:{from: log.did, to: log.client_number}}" class="btn btn-primary btn-reply" data-toggle="m-tooltip" title="Reply">
                                                    Reply
                                                </router-link> -->
...
import TextModal from '../outbound/text_modal'
export default {
  extends: AppComponent,
    ...
    methods: {
        ...
        setReload(){            
            setTimeout(function(){
                $("#Reload").click(function(){
                    this.$router.replace({ name: 'inbound-list', force: true });
                });
             }, 3000);
        },
        resetsearchKey(){
        ...
        bindModalData(data,index){
            if(data){
                this.setMessageRead(data, index);
            }
            this.modalData = data;
        },
        routeToInbox(data,index){
            if(data){
                this.setMessageRead(data, index);
            }
            this.$router.push({name:'InboxList', params:{from: data.did, to: data.client_number}});
        },
        setMessageRead(data,index){
            let href = 'api/schedule-change-status/'+data.olog_time+'/'+data.account_id+'/'+data.did+'/'+data.client_number+'/'+data.callid+'/R';
                        commonLib.blockUI({target: ".m-content", animate: true, overlayColor: 'none', top:'45%'});
                        axios.get(href).then((res) => 
                        {
                            if(this.data.data[index]){
                                this.data.data[index].status = 'R'
                            }
        ...
        fetchTextList(page_url) {
            this.$validator.validateAll().then((result) => {
                if(result == true){
                    page_url = page_url || 'api/inbound-list?page='+this.pagination.current_page+this.searchQueryStr;

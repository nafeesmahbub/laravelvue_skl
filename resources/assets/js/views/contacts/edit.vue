...
export default {
  ...
  mounted(){
      if(this.$route.params.id){ 
          ...
          this.getCountries();
          this.bindCurrentRoute("ContactList");
      }
  },
  methods: {
    setSelected(value) {
    ...
    updateContact() {
        ...
        this.$validator.validateAll().then((result) => { 
            if(result == true){
                ...
                axios.post("api/contacts/"+this.$route.params.id, this.contact).then((res) => 
                {                    
    ...
    getContactById(id) {
        let vm = this;
        var refUrl = 'api/contacts/'+id+'/edit';
        if(typeof commonLib != 'undefined'){
            commonLib.blockUI({target: ".m-content",animate: true,overlayColor: 'none'});
        }
    ...
    getCountries(){      
        ...
        axios.get('api/contact-country-list').then((res) => 
        { 

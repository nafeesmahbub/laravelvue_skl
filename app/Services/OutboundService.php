...
class OutboundService extends AppService
{
    ...
    {        
        ...
        $this->extn = Auth::user()->account_id;
    ...
    public function getPagination($request){
        ...
        $query = DB::table('log_schedule')
                ->leftJoin('log_schedule_contact', function ($join) {
                    $join->on('log_schedule.id', '=', 'log_schedule_contact.schedule_id');
                }) 
                ->where('log_schedule.account_id','=', $this->account_id)
                ->where('log_schedule.sms_from','=', $this->did);
        ...
        if(isset($queryParam['client']) && !empty($queryParam['client']) ){
            ...
            $query->where("log_schedule.phone",$queryParam['client']);
            $hasQuery = true;           
        }
        ...
        if(!$hasQuery){
            $startTime = date('Y-m-d', strtotime('today - 30 days'))." 00:00";
            $endTime = date('Y-m-d')." 23:59";
            $query->whereBetween("log_schedule.start_time",[$startTime,$endTime]);
        }else{
            if($stdate !== false && $endate !== false){
        ...
        $query = $query->orderBy('log_schedule.start_time', 'DESC')->paginate($per_page); 
        ...
        foreach($query as $key => $value){
            ...
            $value->log_time=$this->convertTime(config('app.timezone'), $authUser['timezone'], $value->start_time);
        }
        //dd(DB::getQueryLog());            

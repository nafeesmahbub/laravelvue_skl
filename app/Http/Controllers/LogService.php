...
class LogService extends AppService
{
    ...
    {        
        ...
        $this->did = Auth::user() ? Auth::user()->did : '';
    }
     ...
     * @param string $accountId
     */
	public function getSmsInfo($accountId){
        return $data = DB::table('log_schedule')
        ->select(DB::raw('COALESCE(SUM(num_contacts) - SUM(num_sms_sent),0) AS pending'))        
        ->where('account_id', $accountId)
        ->get();
    }
}
